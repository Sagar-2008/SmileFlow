...
export default function CostCalculator({ config, onOpenBooking }: CostCalculatorProps) {
  ...
  return (
    <section id="calculator" className="py-24 bg-secondary text-white relative overflow-hidden">
      {/* Background Glows */}
      <div className="absolute -top-32 -left-32 w-96 h-96 rounded-full bg-primary/15 blur-[130px] pointer-events-none" />
      <div className="absolute -bottom-32 -right-32 w-96 h-96 rounded-full bg-accent/15 blur-[130px] pointer-events-none" />
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          >
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-theme bg-white/10 border border-white/20 text-white text-xs font-mono uppercase tracking-wider mb-4 font-bold"
            >
            ...
            <motion.h2
              variants={animationsConfig.fadeInUp}
              className="font-display text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight text-white mb-6 leading-tight"
            >
              {calculatorConfig.title}
            </motion.h2>
            <motion.p
              variants={animationsConfig.fadeInUp}
              className="text-base sm:text-lg text-white/80 leading-relaxed mb-8 font-medium"
            >
          ...
          >
            <div className="bg-bg-card text-text-main rounded-theme p-6 sm:p-8 shadow-premium border border-white/20 hover:shadow-2xl transition-all duration-300">
              <h3 className="font-display font-black text-xl text-secondary mb-5 leading-none">
                {calculatorConfig.steps.step1}
              </h3>

              {/* Service Grid Selectors with Hover Scale */}
                ...
                {config.services.map((srv) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-theme border text-left transition-all duration-200 cursor-pointer flex items-center justify-between hover:scale-[1.02] ${
                        isSelected
                          ? "bg-primary/10 border-primary shadow-md ring-2 ring-primary/20"
                          : "bg-bg-base/80 border-border-theme hover:bg-bg-base hover:border-primary/40"
                      }`}
                    >
                      <div>
                        <p className="text-sm font-extrabold text-secondary">{srv.name}</p>
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                {calculatorConfig.financingPeriods.map((period) => (
                  <button
                    ...
                    className={`py-3.5 rounded-theme text-xs sm:text-sm font-black font-mono transition-all duration-200 cursor-pointer text-center hover:scale-105 ${
              ...
              <div className="bg-secondary text-white rounded-theme p-6 flex flex-col sm:flex-row items-center justify-between gap-6 border border-white/10 shadow-lg">
                <div>
                  <span className="text-xs uppercase font-mono tracking-widest text-primary font-extrabold">
                    Estimated Total
                  </span>
                ...
                <div className="text-right sm:border-l sm:border-white/10 sm:pl-6 w-full sm:w-auto">
                  ...
                  <div className="text-4xl font-black font-mono text-primary mt-1 flex items-center justify-end">
                    <span className="text-3xl text-primary mr-0.5">{symbol}</span>
              ...
              <Button
                ...
                className="w-full mt-6 shadow-md hover:scale-[1.03] transition-transform font-black"
              >
